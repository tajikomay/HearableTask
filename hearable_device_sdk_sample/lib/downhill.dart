...
class Downhill extends StatelessWidget {
  const Downhill({super.key});
...
class _Downhill extends StatefulWidget {
  @override
  State<_Downhill> createState() => _DownhillState();
}

class _DownhillState extends State<_Downhill> {
  ...
  DateTime startTime = DateTime.now();  // ゲームがスタートした時間
  DateTime endTime = DateTime.now();    // ゲームが終了した時間
  ...
  void endGame(BuildContext context) {
    ...
    // ゲームオーバー時の経過時間（ミリ秒）を計算
    endTime = DateTime.now();
    int elapsedTime = endTime.difference(startTime).inMilliseconds;
          ...
          content: SingleChildScrollView(
            child: ListBody(
              children: [
                ...
                Text('Time: ${elapsedTime / 1000} seconds', textAlign: TextAlign.center),
              ],
            ),
          ),
          actions: <Widget>[
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('ダウンヒル'),
        centerTitle: true,
        backgroundColor: Color.fromARGB(255, 150, 209, 110),
      ),
      body: Stack(
        children: [
